...
const routes = ( server, mySQL, connection ) => {
	...
	server.get( '/api/getuserlists', ( request, response ) => {
		...
		const listsInserts = [ 'lists.ID', 'eventTime','url','userID', 'lists.name', 'ownerID', 'securityStatus', 'lists.status', 'list_to_users', 'lists', 'listID', 'lists.ID', 'userID', ID, 'status', 'active' ];
